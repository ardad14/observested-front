<template>
    <div class="template col-12 form-signin container justify-content-center align-content-center">
        ...
        <hr>
        <div class="wrapper">
            <h1 class="title text-center">{{ $t("create_place.title") }}</h1>
            <div class="form-group mt-4">
                <label class="label" for="name">{{ $t("create_place.name") }}:</label>
                <input v-model="name" type="text" :class="'form-control ' + formErrors.name" id="name" name="name"
                       :placeholder="$t(`create_place.name`)">
            </div>
            <div class="form-group mt-4">
                <label class="label" for="address">{{ $t("create_place.address") }}:</label>
                <input v-model="address" type="text" :class="'form-control ' + formErrors.address" id="address"
                       name="address" :placeholder="$t(`create_place.address`)">
            </div>
            <div class="form-group mt-4">
                <label class="label" for="open">{{ $t("create_place.open") }}:</label>
                <input class="time" v-model="open" type="time" :class="'form-control ' + formErrors.open"
                       id="open" name="open">
            </div>
            <div class="form-group mt-4">
                <label class="label" for="close">{{ $t("create_place.close") }}:</label>
